...
cd /Users/username/home-assistant-huawei
git pull
```
...
**Test:**
```bash
curl -X POST http://192.168.x.x:8123/api/webhook/git_pull_webhook_secret_12345
```
...
3. Dodaj użytkownika `username` do listy
...
7. Wypełnij:
   - **Subdomain**: `ssh`
   - **Domain**: `example.com` (twoja domena)
   - **Type**: `SSH`
   - **URL**: `localhost:22`
8. Kliknij **Save**
...
```bash
ssh ssh.example.com "echo '✅ SSH działa!' && hostname && whoami"
```
...
Przykład komendy:
```bash
ssh ssh.example.com "cd ~/home-assistant-huawei && git pull"
```
...
3. Sprawdź czy ssh.example.com istnieje w Published Application Routes
...
2. Znajdź rekord `ssh.example.com` (typ CNAME)
3. Usuń go
...
4. Subdomain: `ssh`, Domain: `example.com`
5. **Next**
6. **Add a policy**:
...
ls -la /Users/username/home-assistant-huawei/config/python_scripts/
```
...
4. **Monitor:**
```bash
tail -f /Users/username/home-assistant-huawei/config/home-assistant.log
```
...
```bash
curl -s -H "Authorization: Bearer TOKEN" \
  https://ha.example.com/api/states/binary_sensor.bateria_bezpieczna_temperatura \
  | python3 -m json.tool
```
...
```bash
curl -s -H "Authorization: Bearer TOKEN" \
  https://ha.example.com/api/states/binary_sensor.integracje_status \
...
# Sprawdź Event Log
curl -s -H "Authorization: Bearer TOKEN" \
  https://ha.example.com/api/states/sensor.event_log_ostatnie_zdarzenie

# Sprawdź automatyzację błędów
curl -s -H "Authorization: Bearer TOKEN" \
  https://ha.example.com/api/states/automation.blad_krytyczny_blad_systemu

# Test Telegram
curl -s -X POST https://ha.example.com/api/services/notify/telegram \
...
**Endpointy:**
- **Zewnętrzny:** https://ha.example.com/api/
- **Lokalny:** http://192.168.x.x:8123/api/
...
curl -X GET "https://ha.example.com/api/" \
  -H "Authorization: Bearer <TOKEN>"

# Pobranie stanu encji
curl -s "https://ha.example.com/api/states/sensor.akumulatory_stan_pojemnosci" \
